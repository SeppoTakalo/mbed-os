...
TEST(UDPSocket, recvfrom)
{
    ...
    socket->close();
    CHECK(socket->recvfrom(&addr, 0, 0) == NSAPI_ERROR_NO_SOCKET);

    socket->open((NetworkStack*)&stack);

    stack.return_value = 100;
    CHECK(socket->recvfrom(&addr, 0, 100) == 100);

    socket->set_timeout(0);
    stack.return_value = NSAPI_ERROR_WOULD_BLOCK;
    CHECK(socket->recvfrom(&addr, 0, 100) == NSAPI_ERROR_WOULD_BLOCK);

    socket->set_timeout(-1);
    stack.return_value = NSAPI_ERROR_NO_CONNECTION;
    CHECK(socket->recvfrom(&addr, 0, 100) == NSAPI_ERROR_NO_CONNECTION);
}
